...
import (
	...
	"github.com/evidenceledger/domeonboarding/trustedlists/ishare"
...
func main() {
	...
	opts := &ishare.Options{
		...
		PrivateKeyFilePEM: *privateKeyFilePEM,
		PublicKeyFilePEM:  *publicKeyFilePEM,
	}
	tl, err := ishare.NewIShare(opts)
	if err != nil {
		panic(err)
	}

	at, err := tl.TokenRequest()
	if err != nil {
		panic(err)
	}

	parties, err := tl.PartiesList(at)
	if err != nil {
		panic(err)
	}

	r := djson.GetList(parties, "parties_info.data")

	for _, e := range r {
		fmt.Println(djson.GetString(e, "party_id"))
	}

	response, err := tl.PartyInfo("EU.EORI.NL000000001", at)
	if err != nil {
		panic(err)
	}

	partyInfo := djson.GetMap(response, "party_info")
	if len(partyInfo) == 0 {
		panic("no party info")
	}

	fmt.Println(PrettyFormatMap(partyInfo))
...
func PrettyFormatMap(in any) string {
	theMap, ok := in.(map[string]any)
	if !ok {
		return ""
	}
